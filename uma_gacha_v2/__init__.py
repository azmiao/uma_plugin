...
async def ten_gacha_uma(bot, ev):
    group_id = str(ev.group_id)
    msg = await ten_gacha(group_id, 'uma')
    await bot.send(ev, msg)
...
async def ten_gacha_chart(bot, ev):
    group_id = str(ev.group_id)
    msg = await ten_gacha(group_id, 'chart')
    await bot.send(ev, msg)
...
async def tenjou_gacha_uma(bot, ev):
    group_id = str(ev.group_id)
    msg = await tenjou_gacha(group_id, 'uma')
    await bot.send(ev, msg)
...
async def tenjou_gacha_chart(bot, ev):
    group_id = str(ev.group_id)
    msg = await tenjou_gacha(group_id, 'chart')
    await bot.send(ev, msg)
...
async def full_singer_gacha_chart(bot, ev):
    group_id = str(ev.group_id)
    msg = await full_singer_gacha(group_id, 'chart')
    await bot.send(ev, msg)
...
async def one_gacha(group_id, gacha_type):
...
async def ten_gacha(group_id, gacha_type):
    server, pool_id = await get_pool(group_id)
    gacha = Gacha(pool_id, gacha_type, server)
    ...
    msg = f'[CQ:image,file={result_image}]{msg_com}'
    return msg

async def tenjou_gacha(group_id, gacha_type):
    server, pool_id = await get_pool(group_id)
    gacha = Gacha(pool_id, gacha_type, server)
...
async def full_singer_gacha(group_id, gacha_type):
    server, pool_id = await get_pool(group_id)
    if pool_id == '000000':
