...
async def get_title_url(sup_type):
    ...
    url_region = 'https://wiki.biligame.com/umamusume/支援卡节奏榜合集（繁中服）'
...
async def del_img(sup_type):
    img_path = os.path.join(R.img('umamusume').path, 'uma_support_chart/')
    img_pattern = re.compile(f'繁中服{sup_type}\S+\.png')
    for file in os.listdir(img_path):
        ...
        if file_name:
            ...
            logger.info(f'旧版台服{sup_type}卡节奏榜图片{file_name}删除成功')
...
async def fix_img(img_dict, sup_type):
    ...
    for img_name in list(img_dict[sup_type]['img_data'].keys()):
        ...
        all_height += data_dict[img_name]['height']
    logger.info(f'台服{sup_type}卡节奏榜图片合成成功，即将准备发送')
    return end_img
...
# 返回消息
async def generate_img_tw(sup_type):
    try:
        img_dict, is_update = await generate_url(sup_type)
    except httpx.ConnectTimeout:
        return '请求超时，请重试'
    end_path = os.path.join(R.img('umamusume').path, 'uma_support_chart/end_img_tw/')
    if not os.path.exists(end_path):
        os.mkdir(end_path)
    end_img_path = os.path.join(end_path, f'end_{sup_type}_tw.png')
    # 没有更新时，直接发送本地图片
    if not is_update:
        logger.info(f'台服{sup_type}卡节奏榜没有更新，将发送本地图片文件')
        # 图片丢失就再合成一下，正常情况不会运行到这
        if not os.path.exists(end_img_path):
            logger.info(f'本地台服{sup_type}卡节奏榜图片文件丢失，正在重新合成')
            try:
                end_img = await fix_img(img_dict, sup_type)
            # 如果配置里有信息，但实际没有图片
            except UnboundLocalError as e:
                await del_err_info(sup_type)
                msg_ = f'遇到特殊问题{e}，因此已清除缓存信息，请尝试重新使用命令“台服{sup_type}卡节奏榜”'
                logger.info(msg_)
                return msg_
